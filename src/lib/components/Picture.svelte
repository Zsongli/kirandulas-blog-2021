...
{#if isOpen}
	...
	>
		...
		>
			...
			>
			<img {src} {alt} style="max-height: 95vh; max-width: 90%;" class="mb-5 rounded-md ring-2 ring-black dark:ring-gray-300 m-auto" />
			{#if source}
				<span class="italic text-gray-500 h-auto w-[fit-content] mx-auto block">Forrás: {source}</span>
			{/if}
		</div>
	</div>
{/if}
